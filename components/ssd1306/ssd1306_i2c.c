...
void turn_off_oled(SSD1306_t * dev){
	i2c_cmd_handle_t cmd = i2c_cmd_link_create();
	ESP_LOGE(tag, "TURN OFF");
	i2c_master_start(cmd);
	i2c_master_write_byte(cmd, (dev->_address << 1) | I2C_MASTER_WRITE, true);
	i2c_master_write_byte(cmd, OLED_CONTROL_BYTE_CMD_STREAM, true);
	i2c_master_write_byte(cmd, OLED_CMD_DISPLAY_OFF, true);				// AE
	...
	esp_err_t espRc = i2c_master_cmd_begin(I2C_NUM, cmd, 10/portTICK_PERIOD_MS);
	if (espRc == ESP_OK) {
		ESP_LOGI(tag, "TURN OFF SUCCESS");
	} else {
		ESP_LOGE(tag, "TURN OFF FAIL code: 0x%.2X", espRc);
	}
	i2c_cmd_link_delete(cmd);
...
void turn_on_oled(SSD1306_t * dev){
	i2c_cmd_handle_t cmd = i2c_cmd_link_create();
	i2c_master_start(cmd);
	ESP_LOGE(tag, "TURN ON");
	i2c_master_write_byte(cmd, (dev->_address << 1) | I2C_MASTER_WRITE, true);
		i2c_master_write_byte(cmd, OLED_CONTROL_BYTE_CMD_STREAM, true);
	...
	esp_err_t espRc = i2c_master_cmd_begin(I2C_NUM, cmd, 10/portTICK_PERIOD_MS);
	if (espRc == ESP_OK) {
		ESP_LOGI(tag, "TURN ON SUCCESS");
	} else {
		ESP_LOGE(tag, "TURN ON FAIL code: 0x%.2X", espRc);
	}
	i2c_cmd_link_delete(cmd);
}