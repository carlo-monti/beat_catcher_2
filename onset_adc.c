...
#define GPIO_FOR_ADC_TEST GPIO_NUM_26
...
extern SemaphoreHandle_t onset_array_mutex_handle; /*!< mutex for the access to onset arrauy */  
        ...
        while (1)
        {
            ...
            if(xQueueReceive(onset_adc_task_queue, &event_code, 0)){
                switch(event_code){
                    ...
                    case ONSET_ADC_DISALLOW_ONSET: // called when clock stops
                        bc.there_is_an_onset_out_of_phase = false;
                        allow_onset = false;
                        break;
            ...
            if (ret == ESP_OK)
            {
                for (int i = 0; i < ret_num; i += SOC_ADC_DIGI_RESULT_BYTES * OVERSAMPLING * 2)
                {
                    ...
                    if (delta_y_kick > kick_onset_cfg.delta_threshold)
                    {
                        ...
                        if (current_time_us > kick.last_onset_time + kick_onset_cfg.gate_time_us)
                        {
                            if(allow_onset){
                                ...
                                onsets[bc.most_recent_onset_index].time = US_TO_MS(current_time_us);
                                onsets[bc.most_recent_onset_index].type = KICK;
                                xSemaphoreGive(onset_array_mutex_handle);
                                //ESP_LOGI("onset_adc_task", "KICK!");
                            }else{
                                bc.there_is_an_onset_out_of_phase = true;
                            }
                    ...
                    if (delta_y_snare > snare_onset_cfg.delta_threshold)
                    {
                        ...
                        if (current_time_us > snare.last_onset_time + snare_onset_cfg.gate_time_us)
                        {
                            if(allow_onset){
                                ...
                                bc.most_recent_onset_index = (bc.most_recent_onset_index + 1) % ONSET_BUFFER_SIZE;
                                onsets[bc.most_recent_onset_index].time = US_TO_MS(current_time_us);
                                onsets[bc.most_recent_onset_index].type = SNARE;
                                xSemaphoreGive(onset_array_mutex_handle);
                    ...
                    #endif
                    #ifdef ADC_TEST // xxx togliere
                        if(gpio_get_level(GPIO_FOR_ADC_TEST) == 0){
...
void onset_adc_init()
{
    ...
    xTaskCreate(onset_adc_task, "Onset_Adc_Task", 4096, NULL, ONSET_ADC_TASK_PRIORITY, &onset_adc_task_handle);
}