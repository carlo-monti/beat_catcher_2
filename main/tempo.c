...
TaskHandle_t tempo_task_handle = NULL; /*!< Handle for the tempo task */

const float TEMPO_WEIGHT[17] = {0, 0, 1, 0, 1, 0, 0, 0, 0.92, 0, 0, 0, 0.8, 0, 0, 0, 0.8}; /*!< Weight values for each IOI */ 
const uint16_t SIGMA_TEMPO_WIDTH_FACTOR = 20; /*!< Factor for calculating the max width of the window */  
...
static void tempo_task(void *arg)
{
    ...
    while (1)
    {
        ...
        xTaskNotifyWait(0, 0, &notify_code, portMAX_DELAY);  /*!< Wait for a notify */  
        xSemaphoreTake(bc_mutex_handle, portMAX_DELAY);  /*!< Take the mutex for bc vrb */  
        bool there_is_an_onset = bc.there_is_an_onset;  /*!< Check if there was an onset */  
        uint64_t tau = bc.tau; 
        xSemaphoreGive(bc_mutex_handle);  /*!< Release the mutex for bc vrb */  
        switch (notify_code){
            case TEMPO_START_EVALUATION_NOTIFY:
                if(there_is_an_onset){
                    ...
                    {   
                        interOnsetInterval = (onsets[current_onset].time - onsets[i].time); //calculate IOI for current onset and previous onset (tn - tn-k)
            ...
            case TEMPO_RESET_PARAMETERS:
                ...
                sigma_tempo = round(tau / SIGMA_TEMPO_WIDTH_FACTOR); // xxx mettere 20
                theta_tempo = 0.80;
                break;
            default:
                break;
