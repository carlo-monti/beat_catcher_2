...
static void sync_task(void *arg)
{
    ...
    while (1)
    {
        ...
        case SYNC_START_EVALUATION_NOTIFY:
            if(there_is_an_onset){ // if it has been an onset to sync
                ...
                while(1){
                    ...
                    long error;
                    current_sync_weight = SYNC_WEIGHT[onsets[i].type][bar_position]; // chose between kick or snare
                    error = onsets[i].time - expected_beat;
                    //ESP_LOGI("Sync_task", "Sync_task: NEW ONSET----------------------");
                    //ESP_LOGI("Sync_task", "Tipo: %d",onsets[i].type);
                    // ESP_LOGI("Sync_task", "tau: %lld",tau);
                    // ESP_LOGI("Sync_task", "onset: %lld",onsets[i].time);
                    // ESP_LOGI("Sync_task", "expected: %lld",expected_beat);
                    ...
                    {   
                        ...
                        {// if the current layer is equal or higher than the higherlayer
                            ...
                            if (accuracy >= theta_sync + HEADROOM_VALUE) // if accuracy is higher than the threshold + headroom
                            {
                                ...
                                {
                                    sigma_sync = round(tau / sigma_sync_width); // xxx mettere 20
                                }
                ...
                if (layer == last_synced_layer) // xxx shoulndn't it be >=?
                    {
