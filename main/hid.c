    ...
    while (1)
    {
        ...
        while (xQueueReceive(hid_task_queue, &event_code, 1))
        {
            ...
            case HID_TAP_MODE_SELECT: // Switch to tap mode
                ...
                tap_hits_counter = 0;
                break;
            case HID_PLAY_MODE_SELECT: // Switch to bpm mode
                // ESP_LOGI("hid_task", "Hid switch to bpm");
                break;
            case HID_ENTER_SLEEP_MODE: // Switch to sleep mode
                ESP_LOGI("hid_task", "Hid turn off screen");
